...
setwd("/Volumes/DROPBOX/Dropbox/BERNAT/PHD/01.HAJAR_MOUNTAINS/COLONIZATION/03.Biogeography/Mountain_colonization_Simmaps/Geologic_regions/lowland_midland_highland/")
rm(list = ls())
...
for (i in 1:length(elev_data)) {
  ...
  elev_data[[i]]$elev <- NA
  for (j in 1:nrow(elev_data[[i]])) {
    if (elev_data[[i]]$low[j] == 1) {
      elev_data[[i]]$elev[j] <- 'low'  
    }
    if (elev_data[[i]]$mid[j] == 1) {
      elev_data[[i]]$elev[j] <- 'mid'  
    }
    if (elev_data[[i]]$high[j] == 1) {
      elev_data[[i]]$elev[j] <- 'high'  
    }
  }
...
for (i in 1:length(elev_data)){
  elev_data_def[[i]] <- as.data.frame(elev_data_def[[i]][,2])
  rownames(elev_data_def[[i]]) <- rownames(elev_data[[i]])
}
write_rds(elev_data_def, 'objects/elev_data_def.rds')
