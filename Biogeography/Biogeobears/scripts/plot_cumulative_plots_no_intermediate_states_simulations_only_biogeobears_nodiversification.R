...
diversification_SIM_nodes <- list(vicariance_CE_SIM_nodes_list_cons, 
...
for (i in 1:length(genera)){
  ...
  if (nrow(sim_event_consensus[[i]][[s]]) > 0){
    for (j in 1:length(sim_event_consensus[[i]][[s]]$node)){
      ...
      if (sim_event_consensus[[i]][[s]][j, "vicariance_CE"] == 1 | sim_event_consensus[[i]][[s]][j, "vicariance_CW"] == 1 | sim_event_consensus[[i]][[s]][j, "vicariance_EW"] == 1
...
saveRDS(sim_event_consensus, "Mountain_colonization_area_Biogeobears/3_mountain_blocks_2state/objects/sim_event_consensus_no_diversification.rds")
sim_event_consensus <- readRDS("Mountain_colonization_area_Biogeobears/3_mountain_blocks_2state/objects/sim_event_consensus_no_diversification.rds")
